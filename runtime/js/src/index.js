 ...
 * 
 * Sets up AEMS assets, game loop, input handling, and rendering
 */

import GameEngine from './game-engine.js';
import Renderer from './renderer.js';
import { loadManifestations, createFallbackManifestations } from './asset-loader.js';
...
async function main() {
    ...
    // Load AEMS manifestations
    let manifestations;
    try {
        manifestations = await loadManifestations('classic-1962');
        console.log('[AEMS] Loaded authentic 1962 visual assets');
    } catch (error) {
        console.warn('[AEMS] Failed to load manifestations, using fallback:', error);
        manifestations = createFallbackManifestations();
    }

    // Get canvas and create renderer with manifestations
    ...
    const renderer = new Renderer(canvas, manifestations);
...
// Start the game
main().catch(error => {
    console.error('Failed to start Spacewar!:', error);
});
